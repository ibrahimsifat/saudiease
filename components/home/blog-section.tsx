...
export default function BlogSection({ locale }: { locale: Locale }) {
  ...
  return (
    ...
    >
      {/* Background elements */}
      <div className="absolute top-0 right-0 rtl:left-0 rtl:right-auto w-1/3 h-1/3 bg-primary/5 rounded-full blur-3xl"></div>
      <div className="absolute bottom-0 left-0 rtl:right-0 rtl:left-auto w-1/4 h-1/4 bg-blue-400/5 rounded-full blur-3xl"></div>
              ...
              <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-2">
                  <div className="relative h-64 md:h-full">
                    <Image
                      src={
                        blogPosts.filter((post) => post.featured)[0].image ||
                    ...
                    />
                    <div className="absolute top-4 left-4 rtl:right-4 rtl:left-auto bg-primary text-white text-xs font-bold px-3 py-1 rounded-full">
                      {t("featured")}
                    </div>
                  </div>
                  <div className="p-8 flex flex-col justify-center">
                    <div className="flex items-center text-sm text-gray-500 space-x-4 rtl:space-x-reverse mb-3">
                      <div className="flex items-center">
                        <Calendar className="h-4 w-4 mr-1 rtl:ml-1 rtl:mr-0 text-primary" />
                        <span>
                          {blogPosts.filter((post) => post.featured)[0].date}
                        </span>
                      </div>
                      <div className="flex items-center">
                        <User className="h-4 w-4 mr-1 rtl:ml-1 rtl:mr-0 text-primary" />
                        <span>
                    ...
                    <h3 className="text-2xl font-bold text-gray-800 mb-3 hover:text-primary transition-colors">
                      <Link
                        href={`/blog/${
                          blogPosts.filter((post) => post.featured)[0].slug
                        }`}
                      >
                    ...
                    <Button variant="default" className="w-fit group">
                      ...
                      >
                        {t("readArticle")}
                        <ArrowIcon className="ml-2 rtl:mr-2 rtl:ml-0 h-4 w-4 group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-transform" />
                      </Link>
                    </Button>
                  </div>
                </div>
              ...
              >
                <Card className="h-full overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100 relative">
                  ...
                  <div className="relative h-48">
                    ...
                    />
                    <div className="absolute top-3 right-3 rtl:left-3 rtl:right-auto bg-white/90 backdrop-blur-sm text-primary text-xs px-2 py-1 rounded-full z-20">
                      {post.readTime}
                    </div>
                  </div>
                  <CardHeader className="pt-6">
                    <div className="flex items-center text-sm text-gray-500 space-x-4 rtl:space-x-reverse mb-2">
                      <div className="flex items-center">
                        <Calendar className="h-4 w-4 mr-1 rtl:ml-1 rtl:mr-0 text-primary/70" />
                        <span>{post.date}</span>
                      </div>
                      <div className="flex items-center">
                        <User className="h-4 w-4 mr-1 rtl:ml-1 rtl:mr-0 text-primary/70" />
          ...
          <div className="inline-block p-[1px] rounded-full bg-gradient-to-r from-primary to-blue-600">
            ...
            >
              <Link href="/blog" className="flex items-center">
                {t("viewAllArticles")}
                <ArrowIcon className="ml-2 rtl:mr-2 rtl:ml-0 h-4 w-4" />
              </Link>
            </Button>
          </div>
        </motion.div>
